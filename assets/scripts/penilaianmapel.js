$(function(){
    ...
    function loadSelectMapel(){

        if(kelas_id.val() !== '' && semester_id.val() !== ''){

            mapeldayah_id.find('option').remove().end().find('optgroup').remove().end().append('<option value=""></option>');
            
            $.ajax({
                url: '/iq/mapeldayah/json_combo_mapel',
                type: 'GET',
                data: {
                ...
                success: function(result){
                    ...
                    result.forEach(function(mapel){
                        var format = '';
                        format += '<option value="'+ mapel.id +'">'+ mapel.mapel +'</option>';

                        mapeldayah_id.append(format);
    ...
    function loadTable(){
        ...
        if(tahunajaran_id.val() !== '' && semester_id.val() !== '' && tingkat_id.val() !== '' && kelas_id.val() !== '' && mapeldayah_id.val() !== ''){
            
            $('#row-table').show();

            $.ajax({
                url: '/iq/persentasepenilaiandayah/json',
                type: 'GET',
                data: {
                    mapeldayah_id: mapeldayah_id.val(),
                },
                success: function(result){
                            ...
                            [
                                {
                                    ...
                                    formatter: function(value, row, index){
                                        ...
                                        if(row.penilaiandayah_id == null){
                                            ...
                                            buttons += `<button type="button" class="btn btn-success btn-xs btn-edit" data-penilaian_id="${row.penilaiandayah_id}" data-nama_siswa="${row.nama}"><i class="fa fa-pencil"></i></button>`
                                        }
                                        return buttons;
                    ...
                    [
                        {
                            ...
                            formatter: function(value, row, index){
                                ...
                                if(value == null){
                                    buttons += `<button type="button" class="btn btn-success btn-xs btn-add-persentasepenilaian" data-mapel_id="${row.mapeldayah_id}"><i class="fa fa-plus"></i></button>`
                                }else{
    ...
    mapeldayah_id.on('change', loadTable);
    ...
    $(document).on('click', '.btn-add', function(e){
        ...
        formAdd.find('#mapeldayah_id').val(mapeldayah_id.val());
