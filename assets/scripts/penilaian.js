$(function(){
    ...
    function loadSelectMapel(){
        ...
        if(kelas_id.val() !== '' && semester_id.val() !== ''){
            ...
            mapel_id.find('option').remove().end().find('optgroup').remove().end().append('<option value=""></option>');
            
            $.ajax({
                url: '/iq/mapel/json_combo_mapel',
                type: 'GET',
                data: {
                ...
                success: function(result){
                    ...
                    result.forEach(function(mapel){
                        var format = '';
                        if(mapel.has_child == 0){
                            format += '<option value="'+ mapel.id +'">'+ mapel.mapel +'</option>';
                        }else{
                            format += '<optgroup style="font-weight: normal;" label="'+ mapel.mapel +'">';
                            mapel.sub.forEach(function(sub){
                                format += '<option value="'+ sub.id +'">'+ sub.mapel +'</option>';
                            });
                            format += '</optgroup>';
                        }

                        mapel_id.append(format);
    ...
    function loadTable(){
        ...
        if(tahunajaran_id.val() !== '' && semester_id.val() !== '' && tingkat_id.val() !== '' && kelas_id.val() !== '' && mapel_id.val() !== ''){
                            ...
                            [
                                {
                                    ...
                                    formatter: function(value, row, index){
                                        ...
                                        // Jika mapel adalah gabungan dari mapeldayah maka tampilkan button aksi
                                        if(row.is_gabungan == 0){ 
