...
class Raporuas extends Backend {
	...
	public function download(){
        ...
        */
        ...
        foreach($kelompokmapel as $kelompok){
        	...
        	foreach($mapelparent as $parent){
        		...
        		if($parent->has_child > 0) { 
        			...
        			$objWorksheet->getStyle("C$x:D$x")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        			...
        			foreach($mapelchild as $child){
        				...
        				$objWorksheet->getStyle("AB$x:AX$x")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        				$objWorksheet->getStyle("AB$x:AX$x")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

        				$objWorksheet->mergeCells("AB$x:AD". ($x + 1));
        				$objWorksheet->setCellValue("AB$x", $child->kkm);

        				$objWorksheet->mergeCells("AE$x:AH". ($x + 1));
        				$objWorksheet->setCellValue("AE$x", $child->pengetahuan_nilai_rapor_sekolah);

        				$objWorksheet->mergeCells("AI$x:AL". ($x + 1));
        				$objWorksheet->setCellValue("AI$x", $child->pengetahuan_predikat_huruf);

        				$objWorksheet->mergeCells("AM$x:AP". ($x + 1));
        				$objWorksheet->setCellValue("AM$x", $child->keterampilan_nilai_rapor_sekolah);

        				$objWorksheet->mergeCells("AQ$x:AT". ($x + 1));
        				$objWorksheet->setCellValue("AQ$x", $child->keterampilan_predikat_huruf);

        				$objWorksheet->mergeCells("AU$x:AX". ($x + 1));
        				$objWorksheet->setCellValue("AU$x", $child->sikap_nilai_rapor_sekolah);
        				...
        				$objWorksheet->getStyle("C$x:AX". ($x + 1))->applyFromArray([
				            'borders' => [
				                'allborders' => [
				                    'style' => PHPExcel_Style_Border::BORDER_THIN,
				                    'color' => ['argb', 'FFFF0000']
				        ...
				        $objWorksheet->getStyle("E$x:AA". ($x + 1))->applyFromArray([
				            'borders' => [
				                'inside' => [
        			...
        			$objWorksheet->mergeCells("C$x:D". ($x + 1));
        			$objWorksheet->getStyle("C$x:D". ($x + 1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
        			$objWorksheet->setCellValue("C$x", $no_mapel++);
        			...
        			$objWorksheet->mergeCells("AB$x:AD". ($x + 1));
    				$objWorksheet->setCellValue("AB$x", $parent->kkm);

    				$objWorksheet->mergeCells("AE$x:AH". ($x + 1));
    				$objWorksheet->setCellValue("AE$x", $parent->pengetahuan_nilai_rapor_sekolah);

    				$objWorksheet->mergeCells("AI$x:AL". ($x + 1));
    				$objWorksheet->setCellValue("AI$x", $parent->pengetahuan_predikat_huruf);

    				$objWorksheet->mergeCells("AM$x:AP". ($x + 1));
    				$objWorksheet->setCellValue("AM$x", $parent->keterampilan_nilai_rapor_sekolah);

    				$objWorksheet->mergeCells("AQ$x:AT". ($x + 1));
    				$objWorksheet->setCellValue("AQ$x", $parent->keterampilan_predikat_huruf);

    				$objWorksheet->mergeCells("AU$x:AX". ($x + 1));
    				$objWorksheet->setCellValue("AU$x", $parent->sikap_nilai_rapor_sekolah);
    	...
    	$objWorksheet->mergeCells("AY". ($x - $merge_row_antar_mapel) .":BM". ($x - 1));
    	$objWorksheet->getStyle("AY". ($x - $merge_row_antar_mapel) .":BM". ($x - 1))->applyFromArray([
			'alignment'=> [
		...
		$objWorksheet->setCellValue("AY". ($x - $merge_row_antar_mapel), "Peserta didik sudah menunjukkan kesungguhannya dalam mengamalkan ajaran agama dengan sangat baik. Sudah menunjukkan sikap santun, kerja sama, gotong royong, cinta damai, ramah lingkungan, sikap jujur, peduli dan percaya diri.");
		...
		$objWorksheet->setCellValue("AB$x", "57.64");
		...
		$objWorksheet->mergeCells("AB$x:BM$x");
        $objWorksheet->getStyle("AB$x:BM$x")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$objWorksheet->setCellValue("AB$x", "3.20");
		...
		$objWorksheet->setCellValue("AB$x", "32");
